...
(define-constant ERR-ENTER-NEW-OWNER (err u74))
...
(define-public (set-contract-owner (owner principal))
  (begin
    ...
    (try! (check-is-owner))
      (asserts! (not (is-eq owner (var-get contract-owner))) ERR-ENTER-NEW-OWNER)
...
(define-public (reduce-position (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (percent uint))
    ...
    (let
        ...
        (asserts! (is-ok (check-is-owner)) ERR-NOT-AUTHORIZED)
        (asserts! (as-contract (is-ok (contract-call? token-x-trait transfer dx (as-contract tx-sender) sender none))) TRANSFER-X-FAILED-ERR)
        (asserts! (as-contract (is-ok (contract-call? token-y-trait transfer dy (as-contract tx-sender) sender none))) TRANSFER-Y-FAILED-ERR)
        (map-set pools-data-map { token-x: token-x, token-y: token-y, factor: factor } pool-updated)
...
;; ;;The function calculates how much token Y must be withdrawn to adjust the ratio of the token balances in the pool
